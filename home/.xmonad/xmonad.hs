...
myLayout = spacingRaw True (Border 0 3 3 3) True (Border 2 2 2 2) True $ avoidStruts $ mkToggle (NBFULL ?? NOBORDERS ?? EOT) $ tiled ||| Mirror tiled ||| spiral (6/7)  ||| ThreeColMid 1 (3/100) (1/2) ||| Full
    where
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  -- SUPER + SHIFT KEYS = Modify running things
  ...
  , ((modMask .|. shiftMask , xK_n ), spawn $ "conky-rotate -n")
  , ((modMask .|. shiftMask , xK_p ), spawn $ "conky-rotate -p")
  -- , ((modMask .|. shiftMask , xK_x ), io (exitWith ExitSuccess))

  -- CONTROL + ALT = Start programs
  ...
  , ((controlMask .|. shiftMask , xK_Print ), spawn $ "xfce4-screenshooter -f")
...
--  , ((0, xF86XK_AudioStop), spawn $ "mpc stop")
  ...
  , ((mod1Mask .|. shiftMask,        xK_m), windows W.focusMaster  )
  ...
  , ((controlMask .|. mod1Mask,      xK_Left), sendMessage (IncMasterN 1))

  -- Decrement the number of windows in the master area.
  , ((controlMask .|. mod1Mask,      xK_Right), sendMessage (IncMasterN (-1)))
...
, layoutHook = gaps [(U,35), (D,3), (R,3), (L,3)] $ myLayout ||| layoutHook myBaseConfig
